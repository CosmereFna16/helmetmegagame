...
Players edit their character's name, profile picture, and appearance/bio on `/character`. Profile pictures are stored as bytes on `Character.avatarData`/`avatarMimeType` (resized/compressed with `sharp` on upload) rather than in a third-party bucket, and served back out by the web app itself at `/api/avatar/<characterId>` — the bot builds this into a full URL via `WEB_BASE_URL` when it needs an `avatarURL` for a webhook. Uploading at all is gated on `GameConfig.avatarUploadsEnabled` (Dev Panel, off by default) — `AvatarField.js` hides the file input when it's off, but the real gate is `updateCharacterProfile` (`web/app/(app)/character/actions.js`) ignoring a posted `avatar` field regardless, since a server action is a public endpoint. Off means every character falls through to their letter plaque below; nothing about existing `avatarData` changes when the flag is flipped either way.
...
seat caps that scale with `GameConfig.playerCount`. `db/lib/partySize.js` is
the single source: a base table of **4/8/12/16 per 100 players**, scaled live,
so raising the player-count dial on `/gm/dev` retargets every goal without
touching a line of prose.

Two things about the math are deliberate departures from
`db/lib/roleCapacity.js`, which sits right next to it and does neither, so
don't "fix" one into the other. It **floors** rather than rounds — a party
threshold is a bar you clear, and rounding up would make a 50-player game's
first goal 3 rather than the 2 that "4 per 100, rounded down" means. And it
keeps `roleCapacity`'s `Math.max(1, …)` clamp, for `roleCapacity`'s own
reason: below 25 players `floor` yields 0, and a tier needing nobody would
unlock itself the moment it was authored. Tiers are **1-indexed** at every
boundary (`{partysize:1}` is the first goal) over a 0-indexed table.

The numbers reach a player as a `{partysize:N}` inline token in
`docs/documents.yaml`, resolved exactly the way `{resource:field:tier}` is:
`/api/party-sizes` (`force-dynamic`, unlike the production-rates route, since
`playerCount` is a live dial) ships each tier pre-formatted as `display`,
`PartySizeProvider` fetches it once per page load into context, and the three
renderers turn it into a `PartySizeChip`. Nothing hardcodes a party size in
prose — `docs/threats.md` describes the ladder and points here rather than
printing numbers that would go stale.
...
Two carve-outs. A `{resource:…}` bubble already renders its own glyph via `web/app/components/ResourceChip.js` (`{value} ⬢`), so never write one after it. `PartySizeChip.js` deliberately carries **no** glyph for the mirror-image reason — a party threshold is a count of people, not a currency. And literal syntax a player is meant to *type* (`+3`, `5-12`, `/hunt`) is quoted as-is, never with a glyph appended. `docs/systemdocs/infochannel.yaml`'s glossary line ("the game's central currency are Resources (⬢)") is deliberately left as the one place the glyph is introduced to players.
...
The other registered commands are **not** GM-gated: `/hunt`/`/fish`/`/farm`/`/herd` (see "Food production" above), `/add`/`/remove` (see "Character mentions and private threads" above) and `/persistent` all gate on having a living character instead.

`/persistent` toggles whether the thread it's run in survives the Dawn wipe. Two markers, because the two channel types can't share one: a forum post carries the real "Persistent" (⏰) forum tag, while a private thread lives under a *text* channel that can't have forum tags at all and so takes a **⏰ prefix on its own name**. `db/lib/persistence.js` owns both — constants plus the prefix add/strip/detect helpers — so `/persistent` and `db/lib/dawnWipe.js` can't drift. Surviving means the thread stays and its messages are cleared, same as a tagged forum post has always done; for a private thread the real prize is that the **guest list survives too**, so a standing side-room doesn't need re-inviting every turn. Each use writes a `thread_persistence_changed` audit row. `db/lib/fullWipe.js` (Restart Game) ignores both markers deliberately. Full writeup: `docs/systemdocs/CHANNELS.md` §5. `registerCommands` uses `guild.commands.set()`, a full replace, so adding or removing a command needs no deregistration step.
...
- The player-facing slash commands are `/hunt`/`/fish`/`/farm`/`/herd`, `/add`/`/remove` and `/persistent`; `/gm` and `/message` are GM-only. Several once-imagined ones still don't exist (`/effort`, `/move`, `/resource`, `/zone`, `/tag`) — submitting a Move is still a message in `#turns`, and travel is still the ⚜ picker in `#location`.
