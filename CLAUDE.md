...
A **Gambit**'s d6 now carries a mood modifier: `handleMoveConfirm` stores the raw roll on `Action.diceRoll` and the ±1 separately on `Action.diceModifier` (never baked together — a GM has to be able to tell a modified 5 from a natural 5). See "Requests, Mood, and Desires" below.
