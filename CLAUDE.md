...
Six request types (`RequestType`): `SET_MOOD`, `TRANSFER_RESOURCES`, `ADD_TAG`, `REMOVE_TAG`, `TRANSFER_TAG`, `FULFILL_DESIRE`. All six are server actions in `web/app/(app)/character/requestActions.js`, all re-validate everything the client sent, and all write the effect plus the `Request` row in one `$transaction`.
