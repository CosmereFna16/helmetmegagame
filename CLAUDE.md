...
The single Moves channel is still explicitly configured (`GameConfig.movesChannelId`, set from `/gm/turns` — unlike tupper/summary channels there's no naming convention for it). A message posted there becomes a `PENDING` `Action` (type `MOVE`): `bot/src/lib/moves.js` deletes the original message, records the character's current zone, and DMs the player a confirm prompt with a ✅ reaction already applied. Reacting ✅ is handled by the existing `handleActionConfirm` in `bot/src/events/messageReactionAdd.js` (matched via `Action.confirmDmMessageId`), which rolls a d20 for Moves, DMs the result, and flips the action to `CONFIRMED` — this path already existed before the moves-channel trigger was added and also covers Efforts if a `PENDING` Effort action is ever created some other way.
