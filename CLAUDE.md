...
There is no more Effort/Move split — every turn submission is a **Move**, which is either **Routine** or **Gambit** (`Action.moveKind`; only Gambit rolls a d6) and may additionally be flagged **Opposed** (`Action.opposed`). A message posted in the channel named exactly `turns` becomes a `PENDING_TYPE` `Action` via `bot/src/lib/actionSubmission.js#handleActionSubmission` — it deletes the original message, records the character's current zone, and DMs the player a Routine/Gambit picker. Reacting walks the DM through three steps, each handled in `bot/src/events/messageReactionAdd.js`: `handleMoveKindSelection` (Routine/Gambit → `PENDING_OPPOSED`), `handleOpposedSelection` (Opposed y/n → `PENDING`, sends the ⚜ confirm DM), then the existing `handleActionConfirm` (⚜ → rolls 1d6 only if `moveKind === "GAMBIT"`, strips the confirm reaction, edits the DM to `» *Waiting on adjudication...*`, flips the action to `CONFIRMED`). Requires the `DirectMessageReactions` gateway intent (`bot/src/index.js`) — without it, reactions on DMs never reach the bot at all. Posting a second Move in `#turns` on the same turn is deleted with a DM (`» *You've already sent a Move this turn...*`) rather than recorded.
...
- **Tag-gated roles** — the pattern used for the `#radio` channel (below): a single shared Discord role (not a per-character role) whose membership is synced to whoever currently holds a specific `Tag`, driven from the tag-grant/revoke call sites rather than from Location or Faction state.

### Tag-gated channels (`#radio`)

The `radio` Tag (`RADIO_SLUG`, GM-granted only — not point-buy, see `db/prisma/seed-tags.js`) gates a single text channel, `#radio`, provisioned once via `provisionRadioChannel()` (`web/lib/discordGuild.js`): it creates a shared "Radio Access" Discord role, denies `@everyone` `ViewChannel`/`SendMessages` on the channel, and allows `ViewChannel`/`SendMessages` for that role — IDs are stored on the `GameConfig` singleton (`radioChannelId`/`radioRoleId`), same pattern as `locationPromptChannelId`. `syncRadioAccess(discordUserId, hasTag)` grants/revokes the role via REST (same PUT/DELETE-member-role primitive as `setTurnPingRole`) and is called from `grantTag`/`revokeTag` (`web/app/(app)/gm/dev/actions.js`) whenever the mutated tag's slug is `RADIO_SLUG` — so a GM granting/revoking the Radio tag on a character's raw-edit page is what flips their Discord access, not a separate admin step. This is a template for any future Tag that should gate a Discord channel: add the slug to `db/lib/constants.js`, seed the `Tag`, provision a role+channel pair, and call a `syncXAccess` from the relevant grant/revoke call sites.
