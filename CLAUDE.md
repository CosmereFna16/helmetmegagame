...
Eleven request types (`RequestType`): `SET_MOOD`, `TRANSFER_RESOURCES`, `ADD_TAG`, `REMOVE_TAG`, `CONSUME_TAG`, `TRANSFER_TAG`, `FULFILL_DESIRE`, `CHANGE_WORST_FEAR`, `FULFILL_WORST_FEAR` in `web/app/(app)/character/requestActions.js`, plus `DONATE_BLOOD`/`FEED_PERSON` in `web/app/(app)/lifeweb/requestActions.js` (the Mortus-only Lifeweb panel — blood is worth 40/30/20 by the *target's* Nobility/Courtier tags, capped at a 100 pool, numbers shared with the GM bypass panel via `db/lib/lifeweb.js`). Feeding someone deliberately does **not** kill them: the request flags itself red in the Requests tab and a GM pulls the trigger with the panel's Kill button (`killRequestTarget`). All eleven re-validate everything the client sent, and all write the effect plus the `Request` row in one `$transaction`.
