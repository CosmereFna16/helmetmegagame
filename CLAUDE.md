...
Geography is two levels: `Zone` (e.g. "Town") and `Location` (e.g. "church", nested under a Zone via `Location.zoneId`). Each `Location` maps to a standard Discord layout — one category plus three channels named after the location (`church`, `church-public`, `church-private`).

`docs/locations.yaml` is the **sole master** for the Zone/Location list (`id` slug, `name`, `zone`, free-text `tags`) — there is no manual "add Zone/Location" UI anymore (removed from `/gm/dev/zones`; `db/prisma/seed-zones.js`, the old hardcoded seed script, was deleted along with it). Editing the YAML and running `npm run db:sync-locations` (`db/prisma/sync-locations.js`, a thin wrapper around `db/lib/syncLocations.js#syncLocationsFromYaml`) is the only way new Zones/Locations get created. It's a **manual, terminal-invoked command by default** — no cron, no per-turn hook — but the same `syncLocationsFromYaml(prisma)` function is also called automatically at the end of `wipeGameData`'s "Restart Game" flow (`web/app/(app)/gm/dev/actions.js`), so a reset always lands the game on the canonical location set. The sync is upsert-only by `Location.slug` (never destructive: removing a YAML entry doesn't delete or deprovision anything) and provisions Discord channels — one category plus three channels — for any Location still missing `discordCategoryId`, exactly once. This is **deliberately one-time per Location, never re-synced** — re-running it on an already-provisioned Location is a no-op, so editing a Location's name in the YAML later can't accidentally rename/delete live channels or their message history; the GM Panel's remaining "Provision Discord channels" button on `/gm/dev/zones` (`provisionLocationChannels` in `web/app/(app)/gm/dev/actions.js`, a separate, pre-existing implementation) does the same one-off provisioning for a Location that's stuck unprovisioned. `updateLocation`/`updateZone` on `/gm/dev/zones` still allow renaming a Zone/Location row directly in the DB, but since the YAML is master, a later `db:sync-locations` run (or a game reset) overwrites `name` back to the YAML value — Zone matching is by `name` (no slug on `Zone` yet, a known fragility), Location matching is by `slug`. Locations start unprovisioned (all `discord*Id` fields null) until synced/provisioned.
