...
**Convention: `⬢` is the canonical Resources glyph, and it stands in for the word rather than decorating it.** Prose naming Resources (or the Silo) as a concept takes the plain word and no glyph; wherever a *quantity* is shown, drop the word and write `{number} ⬢` — `0–4 ⬢`, `3 ⬢`, `+5 ⬢`. So a table header reads `Resources` and its cells read `12 ⬢`, never the reverse. Two carve-outs: a `{resource:…}` bubble renders its own glyph (`web/app/components/ResourceChip.js`), so never add one after it; and literal syntax a player types (`+3`, `5-12`, `/hunt`) is quoted as-is. Keep using this glyph on any new Resources/Silo display rather than introducing a different icon. Full rule in the root `CLAUDE.md`.
